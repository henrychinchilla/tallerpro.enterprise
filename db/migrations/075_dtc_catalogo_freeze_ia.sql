-- ═══════════════════════════════════════════════════════
-- 075 — Freeze frame + análisis IA en escaneos, y catálogo global de DTCs
--   dtc_catalogo: 3,071 códigos SAE genéricos (fuente: obd-trouble-codes, MIT).
--   descripcion_es se completa con el diccionario local del módulo y la IA.
-- ═══════════════════════════════════════════════════════

ALTER TABLE public.diagnosticos_obd
  ADD COLUMN IF NOT EXISTS freeze_frame jsonb DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS ia_analisis text;

CREATE TABLE IF NOT EXISTS public.dtc_catalogo (
  codigo         text PRIMARY KEY,
  descripcion_en text,
  descripcion_es text
);

ALTER TABLE public.dtc_catalogo ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS dtc_lectura ON public.dtc_catalogo;
CREATE POLICY dtc_lectura ON public.dtc_catalogo FOR SELECT USING (true);

INSERT INTO public.dtc_catalogo (codigo, descripcion_en) VALUES
