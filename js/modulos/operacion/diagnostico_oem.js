...
(function () {
  ...
  const MARCAS_OEM = ['Toyota','Nissan','Hyundai','Kia','Mitsubishi','Honda','Mazda','Isuzu','Suzuki','Chevrolet / GM','Ford','Volkswagen','Mercedes-Benz','Hino','Fuso','JAC','Foton','International','Freightliner','Kenworth','Peterbilt','Mack','Western Star','Volvo Trucks','Scania','MAN','DAF','Iveco','UD Trucks','Sinotruk','Shacman','Dongfeng','FAW','Yutong','Cummins','Detroit Diesel','PACCAR','Allison','Caterpillar','Komatsu','John Deere','Case','New Holland','JCB','Volvo CE','Hitachi','Liebherr','Develon','SANY','XCMG','Bobcat','Kubota','BMW','Audi','Renault','Peugeot','BYD','Chery','Geely','Changan','Yamaha','Kawasaki','BMW Motorrad','Ducati','Aprilia','KTM','Triumph','Harley-Davidson','Indian','Royal Enfield','Bajaj','TVS','Hero','Benelli','CFMoto','Kymco','SYM','Piaggio','Vespa','Italika','Zontes','Voge','Otro'];
  const ECUS_OEM = ['ECM / PCM (motor)','TCM (transmisión)','ABS / EBCM (frenos)','BCM (carrocería)','SRS / ACM (airbag)','EPS / PSCM (dirección)','IPC (tablero)','HVAC (climatización)','4WD / AWD','Inmovilizador','Gateway','ADAS','SCR / DEF','DPF','EBS / TEBS (remolque)','Retardador','Suspensión','PTO (toma de fuerza)','Control hidráulico','Implementos','IMU (motocicleta)','BMS (batería de tracción)','Inversor','OBC (cargador)','Otro'];
...
=======
        <div class="table-wrap" style="margin-top:9px"><table class="data-table"><thead><tr><th style="width:160px">Marca/modelo</th><th style="width:90px">ECU</th><th>Función</th><th style="width:130px">Evidencia</th><th style="width:90px">Riesgo</th><th style="width:140px;text-align:right">Acciones</th></tr></thead><tbody>${this._oemDefs.length?this._oemDefs.map(d=>`<tr><td><b>${UI.esc(d.marca)}</b> ${UI.esc(d.modelo||'')}</td><td>${UI.esc(d.ecu)}</td><td>${UI.esc(d.nombre)}<br><small>${UI.esc(d.tipo)} ${UI.esc(d.identificador||'')}</small></td><td><span class="badge badge-${d.estado==='verificado'?'green':'amber'}">${UI.esc(d.estado)}</span><br><small>${UI.esc(d.fuente)}</small></td><td>${UI.esc(d.riesgo)}</td><td style="text-align:right">${Modulos.btnAccion('ver',`Modulos.diagnostico_obd.verOEM('${d.id}')`)}${Modulos.btnAccion('editar',`Modulos.diagnostico_obd.editarOEM('${d.id}')`)}${Modulos.btnAccion('eliminar',`Modulos.diagnostico_obd.eliminarOEM('${d.id}','${UI.jsAttr(d.nombre)}')`)}</td></tr>`).join(''):'<tr><td colspan="6">Aún no hay definiciones. Agrega únicamente información con fuente comprobable.</td></tr>'}</tbody></table></div></div>
>>>>>>> Stashed changes
