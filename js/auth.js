/* ── PERMISOS INLINE (no depende de config.js) ── */
const _MODULOS = [
...
const _PERMISOS_ROL = {
  superadmin:   { dashboard:true,  clientes:true,  vehiculos:true,  ordenes:true,  inventario:true,  proveedores:true,  calendario:true,  facturacion:true,  finanzas:true,  rrhh:true,  comunicaciones:true,  config:true,  database:true  },
  admin:        { dashboard:true,  clientes:true,  vehiculos:true,  ordenes:true,  inventario:true,  proveedores:true,  calendario:true,  facturacion:true,  finanzas:true,  rrhh:true,  comunicaciones:true,  config:true,  database:true  },
  gerente_tal:  { dashboard:true,  clientes:true,  vehiculos:true,  ordenes:true,  inventario:true,  proveedores:true,  calendario:true,  facturacion:false, finanzas:false, rrhh:true,  comunicaciones:true,  config:false, database:false },
  gerente_fin:  { dashboard:true,  clientes:false, vehiculos:false, ordenes:false, inventario:false, proveedores:true,  calendario:false, facturacion:true,  finanzas:true,  rrhh:true,  comunicaciones:true,  config:false, database:false },
  recepcionista:{ dashboard:true,  clientes:true,  vehiculos:true,  ordenes:true,  inventario:false, proveedores:false, calendario:true,  facturacion:true,  finanzas:false, rrhh:false, comunicaciones:false, config:false, database:false },
  mecanico:     { dashboard:true,  clientes:false, vehiculos:true,  ordenes:true,  inventario:true,  proveedores:false, calendario:true,  facturacion:false, finanzas:false, rrhh:false, comunicaciones:false, config:false, database:false },
  cliente:      { dashboard:false, clientes:false, vehiculos:false, ordenes:false, inventario:false, proveedores:false, calendario:false, facturacion:false, finanzas:false, rrhh:false, comunicaciones:false, config:false, database:false }
};
...
const Auth = {
  ...
  async loginWithSupabase(email, password, tenantSlug = null) {
    ...
    /* 2. Cargar perfil de public.usuarios */
    let perfil = null;
    try {
      const { data: p } = await sb.from('usuarios').select('*').eq('id', userId).maybeSingle();
      perfil = p;
    } catch(e) { console.warn('perfil error:', e); }

    /* 3. Cargar tenant */
    let tenant = null;
    if (perfil?.tenant_id) {
      try {
        const { data: t } = await sb.from('tenants').select('*').eq('id', perfil.tenant_id).maybeSingle();
        tenant = t;
      } catch(e) { console.warn('tenant error:', e); }
    }
    /* Fallback tenant */
    if (!tenant) {
      try {
        const { data: t } = await sb.from('tenants').select('*').limit(1).maybeSingle();
        tenant = t;
      } catch(e) { console.warn('tenant fallback error:', e); }
    }

    /* 4. Establecer sesión */
    Auth.tenant = tenant || { id: null, name: 'TallerPro', slug: 'demo' };
    Auth.user = perfil || {
      id: userId, email,
    ...
    sb.from('usuarios').update({ ultimo_login: new Date().toISOString() })
      .eq('id', userId).then(() => {});

    /* 6. Verificar primer login */
    const debeCambiar = perfil?.debe_cambiar_password === true;
  ...
  async _loadProfile(userId, email, tenantSlug = null) {
    try {
      ...
      if (data && !error) {
        ...
        Auth.user.permisos_custom = data.permisos_custom || null;
...
const AUTH_UI = {
  ...
  mostrarCambioPasswordObligatorio() {
    const loginBox = document.querySelector('.login-box') || document.getElementById('login-screen');
    if (!loginBox) { renderLoginView('recovery-new-pass'); return; }

    const box = loginBox.querySelector ? loginBox.querySelector('.login-box') || loginBox : loginBox;
    box.innerHTML = `
      <div style="text-align:center;margin-bottom:20px">
        <div style="font-size:40px;margin-bottom:8px">🔑</div>
        <h2 style="font-family:'Bebas Neue',sans-serif;font-size:24px;color:var(--amber)">
          Cambia tu Contraseña
        </h2>
        <p style="font-size:12px;color:var(--text2)">
          Esta es tu primera vez entrando al sistema.<br>Debes crear una contraseña personal.
        </p>
      </div>
      <div class="login-form">
        <div class="form-group">
          <label class="form-label">Nueva Contraseña *</label>
          <div style="position:relative">
            <input class="form-input" id="oblig-pass1" type="password"
                   placeholder="Mínimo 8 caracteres" style="padding-right:40px"
                   oninput="AUTH_UI.checkPassStrengthId('oblig-pass1','oblig-strength-bar')">
            <button onclick="UI._togglePass('oblig-pass1',this)"
          ...
          <div style="height:4px;border-radius:2px;background:var(--border);margin-top:6px;overflow:hidden">
            <div id="oblig-strength-bar" style="height:100%;width:0%;transition:width .3s,background .3s"></div>
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Confirmar Contraseña *</label>
          <input class="form-input" id="oblig-pass2" type="password" placeholder="Repetir contraseña">
        </div>
        <button class="btn btn-amber" style="width:100%" onclick="AUTH_UI.guardarPasswordObligatorio()">
          Guardar y Entrar →
        </button>
      </div>`;
  },

  checkPassStrengthId(inputId, barId) {
  ...
  async guardarPasswordObligatorio() {
    ...
    /* Actualizar en public.usuarios — limpiar password_temp */
    if (Auth.user?.id) {
      ...
      Auth.user.password_temp = null;
    }

    if (error) console.warn('updateUser:', error.message);
    UI.toast('¡Contraseña guardada! Bienvenido ✓');
    setTimeout(() => startApp(), 800);
  },
...
async function doLogin() {
  ...
  /* Buscar taller si no está seleccionado */
...
const USER_MGMT = {
  ...
  async actualizar(userId, fields) {
    ...
    const { error } = await getSupabase().from('usuarios')
      ...
      .eq('id', userId);
    if (error) console.error('USER_MGMT.actualizar:', error.message);
    return !error;
  },
...
Pages.modalGestionUsuarios = async function () {
  ...
  const ROLES_DISP = Object.entries(ROLES).filter(([k]) => k !== 'superadmin');
...
Pages.modalInvitarUsuario = function () {
  const ROLES_DISP = Object.entries(ROLES).filter(([k]) => k !== 'superadmin');
  UI.openModal('＋ Nuevo Usuario del Sistema', `
    <div class="alert alert-amber mb-4">
      <div class="alert-icon">🔑</div>
      <div class="alert-body" style="font-size:12px">
        <b>Sin envío de correo.</b> El usuario inicia sesión con la contraseña temporal
        y al primer ingreso debe cambiarla obligatoriamente.
      </div>
    </div>
    <div class="form-row">
      ...
      <div class="form-group"><label class="form-label">Contraseña Temporal *</label>
        <div style="position:relative">
          <input class="form-input" id="inv-pass" type="password"
                 placeholder="Mínimo 8 caracteres" style="padding-right:40px">
          <button type="button" onclick="UI._togglePass('inv-pass',this)"
    ...
    <div class="modal-footer">
      <button class="btn btn-ghost" onclick="UI.closeModal()">Cancelar</button>
      <button class="btn btn-amber" onclick="Pages.invitarUsuario()">Crear Usuario</button>
    </div>`
  );
};
...
Pages.invitarUsuario = async function () {
  ...
  /* Guardar en public.usuarios con contraseña temporal — sin enviar correo */
  ...
  getSupabase().auth.signUp({ email, password: pass, options: { data: { nombre, rol } } })
    .then(({ error: authErr }) => {
      if (authErr) console.warn('Auth signup background:', authErr.message);
    });
  ...
  UI.openModal('✏️ Editar: '+u.nombre, `
    ...
    <div style="border-top:1px solid var(--border);padding-top:14px;margin-top:4px">
      <div style="font-weight:700;font-size:11px;color:var(--text3);letter-spacing:.08em;text-transform:uppercase;margin-bottom:10px">
        🔐 Acceso a Módulos
        <span style="font-weight:400;font-size:10px;margin-left:6px">(verde = activo · gris = sin acceso)</span>
      </div>
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px" id="permisos-grid-${u.id}">
        ${_MODULOS.map(m=>{
          const baseRol = (_PERMISOS_ROL[u.rol]||{});
          const custom = (u.permisos_custom||{})[m.id];
          const activo = custom !== undefined ? custom : (baseRol[m.id]||false);
          return '<label style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:var(--surface2);border-radius:6px;cursor:pointer;font-size:12px">' +
            '<input type="checkbox" data-modulo="' + m.id + '" ' + (activo?'checked':'') + ' style="accent-color:var(--cyan)">' +
    ...
    <div class="modal-footer">
      <button class="btn btn-ghost" onclick="UI.closeModal()">Cancelar</button>
      <button class="btn btn-ghost btn-sm" onclick="Pages.modalResetPassword && Pages.modalResetPassword('${userId}','${u.nombre}')">🔑 Reset</button>
...
Pages.actualizarUsuario = async function (userId) {
  ...
  /* Recopilar permisos personalizados del grid */
  const checkboxes = document.querySelectorAll('#permisos-grid-' + userId + ' input[type=checkbox]');
  let permisos_custom = null;
  if (checkboxes.length > 0) {
    permisos_custom = {};
    const baseRol = PERMISOS_ROL[rol] || {};
    checkboxes.forEach(cb => {
      const mod = cb.dataset.modulo;
      const val = cb.checked;
      /* Solo guardar si difiere del valor base del rol */
      if (val !== (baseRol[mod] ?? false)) {
        permisos_custom[mod] = val;
      }
    });
    if (Object.keys(permisos_custom).length === 0) permisos_custom = null;
  }
  ...
  const campos = {
    ...
    permisos_custom: permisos_custom,
    updated_at: new Date().toISOString()
  };
