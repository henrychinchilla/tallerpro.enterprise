...
/* ══════════════════════════════════════════════════════
   MIS ÓRDENES — Vista exclusiva para clientes
   Solo muestra el estado de sus OTs activas
══════════════════════════════════════════════════════ */
Pages['mi_ot'] = async function () {
  const el = document.getElementById('page-content');
  el.innerHTML = '<div class="page-header"><h1 class="page-title">🔍 Mis Órdenes</h1></div><div class="page-body"><div class="text-muted">Cargando...</div></div>';
  ...
  const { data: cliente } = await getSupabase()
    ...
    .eq('tenant_id', tid).eq('email', email).maybeSingle();

  if (!cliente) {
    el.innerHTML = `<div class="page-header"><h1 class="page-title">🔍 Mis Órdenes</h1></div>
      <div class="page-body">
        <div class="alert alert-cyan"><div class="alert-icon">ℹ️</div>
        <div class="alert-body">No se encontró un perfil de cliente asociado a tu correo.</div></div>
      </div>`;
    return;
  }

  /* Cargar OTs activas del cliente */
  const { data: ordenes } = await getSupabase()
    .from('ordenes')
    .select('*, vehiculos(placa,marca,modelo,anio,color)')
    .eq('tenant_id', tid)
    .eq('cliente_id', cliente.id)
    .not('estado', 'in', '("entregado","cancelado")')
    .order('created_at', { ascending: false });

  const { data: historial } = await getSupabase()
    .from('ordenes')
    .select('*, vehiculos(placa,marca,modelo,anio,color)')
    .eq('tenant_id', tid)
    .eq('cliente_id', cliente.id)
  ...
  const ESTADOS = {
    recibido:    { label:'📥 Recibido',        color:'var(--text3)',  pct:10  },
    diagnostico: { label:'🔍 En Diagnóstico',  color:'var(--cyan)',   pct:30  },
    en_proceso:  { label:'🔧 En Proceso',       color:'var(--amber)',  pct:60  },
    listo:       { label:'✅ Listo para retirar',color:'var(--green)', pct:90  },
    entregado:   { label:'🏁 Entregado',        color:'var(--green)',  pct:100 },
    cancelado:   { label:'❌ Cancelado',        color:'var(--red)',    pct:0   }
  };

  const renderOT = (o, activa=true) => {
    const v   = o.vehiculos;
    const est = ESTADOS[o.estado] || { label:o.estado, color:'var(--text2)', pct:50 };
    return `
      <div class="card" style="margin-bottom:16px;${activa?'border-color:'+est.color+';border-width:2px':'opacity:.7'}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
          <div>
            <div style="font-weight:700;font-size:15px;color:var(--amber)">${o.num}</div>
            <div style="font-size:12px;color:var(--text3)">Ingreso: ${o.fecha_ingreso||o.created_at?.slice(0,10)||'—'}</div>
          </div>
          <span style="font-size:13px;font-weight:700;color:${est.color}">${est.label}</span>
        </div>
        ${v ? `<div style="font-size:13px;color:var(--text2);margin-bottom:10px">
          🚗 ${v.marca} ${v.modelo} ${v.anio} · ${v.placa} · ${v.color||''}
        </div>` : ''}
        <div style="font-size:13px;color:var(--text2);margin-bottom:${activa?'14px':'0'}">
          ${o.descripcion||'—'}
        </div>
        ${activa ? `
        <div style="background:var(--surface2);border-radius:8px;overflow:hidden;height:8px;margin-bottom:6px">
          <div style="height:100%;width:${est.pct}%;background:${est.color};transition:width .5s;border-radius:8px"></div>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text3)">
          <span>Recibido</span><span>Diagnóstico</span><span>En Proceso</span><span>Listo</span><span>Entregado</span>
        </div>` : ''}
        ${o.fecha_estimada && activa ? `<div style="font-size:11px;color:var(--text3);margin-top:8px">📅 Entrega estimada: <b>${o.fecha_estimada}</b></div>` : ''}
        ${o.total > 0 ? `<div style="font-size:12px;color:var(--text3);margin-top:4px">💰 Total: <b style="color:var(--amber)">${UI.q(o.total)}</b></div>` : ''}
      </div>`;
  };
  ...
  el.innerHTML = `
    <div class="page-header">
      <div>
        <h1 class="page-title">🔍 Mis Órdenes</h1>
        <p class="page-subtitle">// ${cliente.nombre}</p>
      </div>
    </div>
    <div class="page-body">
      <div class="card card-amber" style="margin-bottom:20px">
        <div class="alert-icon" style="font-size:12px;color:var(--text3)">
          Actualiza la página para ver los cambios más recientes en el estado de tu vehículo.
        </div>
      </div>

      <div style="font-weight:700;font-size:13px;color:var(--amber);margin-bottom:12px">
        🔧 Órdenes Activas (${(ordenes||[]).length})
      </div>
      ${(ordenes||[]).length
        ? (ordenes||[]).map(o => renderOT(o, true)).join('')
        : '<div class="text-muted" style="padding:20px;text-align:center">No tienes órdenes activas en este momento</div>'
      }

      ${(historial||[]).length ? `
      <div style="font-weight:700;font-size:13px;color:var(--text3);margin:20px 0 12px">
        📋 Historial Reciente
      </div>
      ${(historial||[]).map(o => renderOT(o, false)).join('')}` : ''}
    </div>`;
};
