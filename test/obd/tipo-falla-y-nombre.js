...
(async () => {
  ok('lee el nombre propio', (await M._identidadModulo({req:0x745,resp:0x74D})).nombre === 'ABS_ESP');

  /* La REFERENCIA (F187, numero de pieza) es lo que de verdad identifica al
     modulo: la mayoria no publica nombre y si publica su numero de repuesto. */
  const refer = t => [0x62,0xF1,0x87].concat([...t].map(c=>c.charCodeAt(0)));
  M._udsPedir = async (req,resp,tx) => (tx[1]===0xF1 && tx[2]===0x87) ? refer('58920-G6300') : null;
  const soloRef = await M._identidadModulo({req:0x7B3});
  ok('lee la referencia aunque no haya nombre',
     soloRef.referencia === '58920-G6300' && !soloRef.nombre);

  M._udsPedir = async () => [0x62,0xF1,0x97, 0x00,0x01,0xFF];      // basura binaria
  ok('rechaza basura binaria', await M._identidadModulo({req:0x745}) === null);
  M._udsPedir = async () => [0x62,0xF1,0x97, 0x41,0x42];           // "AB", muy corto
  ok('rechaza nombre muy corto', await M._identidadModulo({req:0x745}) === null);
  M._udsPedir = async () => [0x7F,0x22,0x31];                      // negativa
  ok('rechaza respuesta negativa', await M._identidadModulo({req:0x745}) === null);
  M._udsPedir = async () => null;
  ok('rechaza sin respuesta', await M._identidadModulo({req:0x745}) === null);

  /* La sugerencia por grupo de numero de pieza es una PISTA, nunca el nombre. */
  ok('sugiere el sistema por el grupo de la referencia',
     M._sugerenciaPorReferencia('58920-G6300').sistema === 'Frenos (ABS / ESC)');
  ok('no sugiere nada con una referencia que no es de ese formato',
     M._sugerenciaPorReferencia('ABC123') === null && M._sugerenciaPorReferencia('') === null);
  ok('un grupo desconocido no se inventa', M._sugerenciaPorReferencia('11111-AAAA') === null);
